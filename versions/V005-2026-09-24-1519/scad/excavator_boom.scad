...
part = "assembly"; // [assembly, flat, tubeface, flatbrand, tubebrand, boom, boom_tubes, boom_gussets, boom_bracket_D, boom_bracket_F, boom_foot_boss, boom_fork_B, stick, stick_tube, stick_cheeks, stick_bracket_H, stick_tip, rocker, link, bucket, bucket_sides, bucket_shell, bucket_top, bucket_edge, bucket_ears, bucket_wear, post, overlap, collide, view_all]
...
show_post = true;
show_ground = true;
// Логотип на трубах стріли й рукояті, плитах колони і боковинах ковша (наліпка: у металі не ріжеться)
show_brand = true;
...
module boom_assembly() {
    if (show_boom) { boom_body(); boom_bend_gussets(); boom_cyl_bracket_D(); boom_cyl_bracket_F(); boom_foot_boss(); boom_fork_B();
                     rotate([0, -boom_alpha1, 0]) brand_decal("boom_seg1"); }   // система tube_local → система стріли
}
...
module stick_assembly() {
    if (show_stick) { stick_body(); stick_cheeks(); stick_cyl_bracket_H(); stick_tip_bosses(); brand_decal("stick"); }
}
...
module bucket_assembly() { bucket_sides(); bucket_shell(); bucket_top(); bucket_edge(); bucket_ears(); bucket_wear(); brand_decal("bk_side"); }
...
module post_schematic() {
    ...
    for (s = sides()) translate([0, s*(w/2 + 6), 0]) brand_decal("post_plate");
    pin([0, 0, 0], pin_A, w + 40);
    pin(p3(C_w), boom_cyl_pin, w + 40);
}

// =============================================================================
//  ЛОГОТИП. У металі — наліпка або фарба (в DXF, BOM і STL окремих деталей його немає);
//  у друкованому наборі 1:5 — виїмка 0.4 мм (print3d-parts/brand.scad бере місця ЗВІДСИ).
//  Одна таблиця на все: 3D-сторінки, рендери, аркуші розкладки і виїмки набору.
//
//  Полігони — scad/brand/*.scad, у друкованих мм (знак Ø18 і Ø12): штрихи й стінки між
//  виїмками там не тонші за 0.8 мм — дві лінії сопла. Тому розміри тут — рівно ×5 від них.
//
//  Усі грані зі знаком перпендикулярні Y (бічна стінка труби, плити, боковини). Кут —
//  як бачить глядач ЗОВНІ, з верхом деталі вгору, проти годинникової стрілки:
//    - труби (сегмент 1 стріли, рукоять) — обидві бічні стінки, рядок уздовж труби (0°: кожному
//      глядачеві до його правої руки — на лівому боці до колони, на правому до ковша);
//      місце — між накладкою осі A і щоками перелому, між щокою п'яти й накладкою кінця;
//    - плити колони — обидві грані обох плит (у наборі вони однакові ×2), лише знак,
//      у найширшому місці профілю між отворами A і C;
//    - боковини ковша — зовнішня грань кожної: рядок паралельно дну, верх знака — до пащі
//      (як на ковші, що стоїть на дні; у позі за замовчуванням дно відхилене від рівня на
//      5.5°). У системі ковша (x — до зуба) звідси 180° ± bucket_floor_angle: + на лівій
//      (+Y), − на правій. Не «дзеркально»: кожному глядачеві рядок іде до ЙОГО правої руки,
//      тобто на лівому боці — до колони, на правому — від неї. Центр — де до кромок найдалі.
//  Центри підібрані на друкованих деталях (пошук найбільшого поля до кромок і накладних
//  деталей) і переведені в мм моделі.
// =============================================================================
...
BRAND_ON_YELLOW = [0.05, 0.05, 0.05];      // лого монохромне: чорне на світлому,
BRAND_ON_DARK   = [0.96, 0.96, 0.96];      // біле на темному (плити колони)
BRAND_PROUD = 0.5;                         // наліпка над поверхнею, мм — лише щоб її було видно
BRAND_GAP = 0.05;                          // і НЕ торкається її: злиття по спільній площині коштувало WASM-сторінці ще 0.6 с
// [ключ, бік (+1 — зовнішня нормаль +Y, −1 — −Y), y грані, [x, z] центра композиції, кут, що, Ø знака, колір]
//   ключ = система координат: boom_seg1 — tube_local("boom_seg1"), stick — рукоять,
//   post_plate — одна плита колони з центром товщини в y = 0, bk_side — ківш
brand_marks = [
    ["boom_seg1",  1, boom_tube_w/2,  [250, 0],      0, "horizontal", 90, BRAND_ON_YELLOW],
    ["boom_seg1", -1, -boom_tube_w/2, [250, 0],      0, "horizontal", 90, BRAND_ON_YELLOW],
    ["stick",      1, stick_tube_w/2, [550, 0],      0, "horizontal", 60, BRAND_ON_YELLOW],
    ["stick",     -1, -stick_tube_w/2, [550, 0],     0, "horizontal", 60, BRAND_ON_YELLOW],
    ["post_plate", 1, 6,              [51, -230.5],  0, "mark",       90, BRAND_ON_DARK],
    ["post_plate", -1, -6,            [51, -230.5],  0, "mark",       90, BRAND_ON_DARK],
    ["bk_side",    1, bk_w_in/2 + bucket_side_t,    [228, 159], 180 + bucket_floor_angle, "horizontal", 60, BRAND_ON_YELLOW],
    ["bk_side",   -1, -(bk_w_in/2 + bucket_side_t), [228, 159], 180 - bucket_floor_angle, "horizontal", 60, BRAND_ON_YELLOW],
];
module brand_shape(what, D) {
    if      (what == "horizontal" && D == 90) scale(5) logo_horizontal_18();
    else if (what == "horizontal" && D == 60) scale(5) logo_horizontal_12();
    else if (what == "mark"       && D == 90) scale(5) logo_mark_18();
    else echo(str("!!! бренд: немає полігонів для ", what, " Ø", D));
}
// габарит композиції у її власних координатах (центр знака = 0, слово праворуч)
function brand_bounds(what, D) = 5 * (what == "mark" ? [-9, -9, 9, 9] : D == 90 ? logo_horizontal_18_bounds : logo_horizontal_12_bounds);
// Тіло знаків деталі key: від a мм під поверхнею до b мм над нею (уздовж зовнішньої нормалі).
// side = 0 — усі грані, ±1 — лише грань із цим боком. Композиція центрується за габаритом.
// 2D (u — праворуч для глядача, v — угору) → модель: x = −side·u, z = v, y — уздовж нормалі.
module brand_body(key, a, b, side = 0) {
    for (m = brand_marks) if (m[0] == key && (side == 0 || m[1] == side)) {
        s = m[1]; c = m[3]; bb = brand_bounds(m[5], m[6]);
        multmatrix([[-s, 0, 0, c[0]], [0, 0, s, m[2] - s*a], [0, 1, 0, c[1]], [0, 0, 0, 1]])
            linear_extrude(a + b) rotate([0, 0, m[4]]) translate(-[(bb[0] + bb[2])/2, (bb[1] + bb[3])/2]) brand_shape(m[5], m[6]);
    }
}
// Наліпка для показу: лише у вузлі цілком ($sel == "") — в окремих деталях і DXF її немає
module brand_decal(key, side = 0) {
    if (show_brand && $sel == "") for (m = brand_marks) if (m[0] == key && (side == 0 || m[1] == side))
        color(m[7]) brand_body(key, -BRAND_GAP, BRAND_PROUD, m[1]);
}
...
// ---- Логотип на ескізах (tools/bom_drawings.py → drawings/parts.pdf; у DXF його немає): та сама проєкція, що
//      flat() і tube_face_2d(), тобто вигляд з боку −Y. Пластина — знак на грані −Y (її видно на ескізі);
//      труба — знак своєї стінки: "left" — грань +Y (на ескізі вона видна зсередини), "right" — грань −Y.
module flat_brand_2d(name) {
    i = search([name], flat_parts)[0];
    if (!(is_undef(i) || i == [])) projection() rotate([-90, 0, 0]) rotate([0, flat_parts[i][4], 0]) brand_body(name, 1, 1, -1);
}
module tube_brand_2d(name, face) {
    if (face == "left" || face == "right") projection() rotate([-90, 0, 0]) brand_body(name, 1, 1, face == "left" ? 1 : -1);
}

if (part == "assembly") assembly();
else if (part == "flat") flat(flat_name);
else if (part == "tubeface") tube_face_2d(tube_name, tube_face);
else if (part == "flatbrand") flat_brand_2d(flat_name);
else if (part == "tubebrand") tube_brand_2d(tube_name, tube_face);
