...
module pin(p, d, len) { color([0.80, 0.83, 0.85]) translate(p) rotate([90, 0, 0]) cylinder(d = d, h = len, center = true); }
...
module hyd_cyl_part(which, bore, rod, closed, stroke, pin, wall = 5, eye_len = 45, rod_vis = undef, col_body = [0.18, 0.31, 0.56]) {
    body_od = bore + 2*wall;
    ...
    if (which == "body") {
        // вушко бази
        color([0.29, 0.31, 0.33]) rotate([90, 0, 0]) difference() { cylinder(d = eye_od, h = eye_w, center = true); cylinder(d = pin, h = 100, center = true); }
        // шийка вушка — плоска, завширшки як вушко (входить у вилку); корпус Ø починається на відстані eye_len від осі пальця
        color([0.29, 0.31, 0.33]) translate([pin/2, -eye_w/2, -eye_od*0.35]) cube([eye_len - pin/2 + 1, eye_w, eye_od*0.7]);
        // корпус
        ...
        // шток
        color([0.92, 0.94, 0.96]) translate([-eye_len - rv, 0, 0]) rotate([0, 90, 0]) cylinder(d = rod, h = rv + 1);
        // вушко штока
        color([0.29, 0.31, 0.33]) rotate([90, 0, 0]) difference() { cylinder(d = eye_od, h = eye_w, center = true); cylinder(d = pin, h = 100, center = true); }
        color([0.29, 0.31, 0.33]) translate([-eye_len, -eye_w/2, -eye_od*0.35]) cube([eye_len - pin/2, eye_w, eye_od*0.7]);
    }
}
...
module hyd_cylinder(P1, P2, bore, rod, closed, stroke, pin, wall = 5, eye_len = 45) {
    ...
    translate(p3(P1)) rotate([0, -a, 0]) {
        hyd_cyl_part("body", bore, rod, closed, stroke, pin, wall, eye_len, col_body = ok ? [0.18, 0.31, 0.56] : [0.9, 0.2, 0.2]);
...
module boom_body() {
    if (sel("tube")) color([0.85, 0.45, 0.10]) { boom_seg1(); boom_seg2(); }
}
...
module boom_foot_boss() {
    if (sel("A_bushing")) color([0.54, 0.29, 0.12]) bushing([0, 0, 0], bush_od_main, pin_A, boom_foot_boss_len);
...
module stick_body() {
    if (sel("tube")) color([0.85, 0.45, 0.10]) difference() {
...
module stick_cheeks() {
    ...
    if (sel("B_bushing")) color([0.54, 0.29, 0.12]) bushing([0, 0, 0], bush_od_main, pin_B, stick_pack_w);
    // приварені розпірні втулки осі G між щоками: проміжок під вушко = clevis_gap_25
    if (sel("G_boss")) color([0.54, 0.29, 0.12]) for (s = sides()) translate(p3(G_s)) rotate([90, 0, 0])
...
module stick_tip_bosses() {
    ...
    if (sel("E_bushing")) color([0.54, 0.29, 0.12]) bushing(p3(E_s), bushE_od, pin_E, len);
    if (sel("R_bushing")) color([0.54, 0.29, 0.12]) bushing(p3(R_s), bush_od_small, pin_R, len);
    h2 = stick_tube_h/2;
...
module washers_B() {
    ...
    if (t > 0.2) color([0.80, 0.83, 0.85]) for (s = sides()) translate([0, s*(stick_pack_w/2 + t/2), 0]) rotate([90, 0, 0])
...
module rocker_link_pts(R, J, Q, which = "both") {
    if (!is_undef(J)) {
        ...
        if (which != "link") {
            ...
            if (sel("J_boss")) color([0.54, 0.29, 0.12]) for (s = sides()) translate(p3(J)) rotate([90, 0, 0]) translate([0, 0, s*(clevis_gap_25/2 + boss_J/2)])
        ...
        if (which != "rocker")
            if (sel("link_boss")) color([0.54, 0.29, 0.12]) for (s = sides()) for (P = [J, Q]) bushing([P[0], s*(w_l/2 + plate_boss + link_boss_len/2), P[1]], link_boss_od, pin_JQ + 0.5, link_boss_len);
...
module bucket_ears() {      // вуха стоять на накладці; зовнішні бобишки на осі E; розпірна втулка осі Q між вухами
    ...
    if (sel("bk_boss_E")) color([0.54, 0.29, 0.12]) for (s = sides()) bushing([0, s*(gap/2 + t + bucket_boss_E_len/2), 0], bucket_boss_E_od, pin_E + 0.5, bucket_boss_E_len);
    if (sel("bk_spacer_Q")) color([0.54, 0.29, 0.12]) bushing(p3(bucket_ear), bush_od_small, pin_JQ + 0.5, gap);
}
