...
module pin(p, d, len) { color("silver") translate(p) rotate([90, 0, 0]) cylinder(d = d, h = len, center = true); }
...
module hyd_cyl_part(which, bore, rod, closed, stroke, pin, wall = 5, eye_len = 45, rod_vis = undef, col_body = [0.25, 0.35, 0.6]) {
    body_od = bore + 2*wall;
    ...
    if (which == "body") {
        // вушко бази
        color("dimgray") rotate([90, 0, 0]) difference() { cylinder(d = eye_od, h = eye_w, center = true); cylinder(d = pin, h = 100, center = true); }
        // шийка вушка — плоска, завширшки як вушко (входить у вилку); корпус Ø починається на відстані eye_len від осі пальця
        color("dimgray") translate([pin/2, -eye_w/2, -eye_od*0.35]) cube([eye_len - pin/2 + 1, eye_w, eye_od*0.7]);
        // корпус
        color(col_body) translate([eye_len, 0, 0]) rotate([0, 90, 0]) cylinder(d = body_od, h = body_L);
        // порти
        color("goldenrod") for (x = [eye_len + 25, eye_len + body_L - 25]) translate([x, 0, body_od/2]) cylinder(d = 16, h = 14);
    } else {
        rv = is_undef(rod_vis) ? stroke + 50 : rod_vis;
        // шток
        color("lightsteelblue") translate([-eye_len - rv, 0, 0]) rotate([0, 90, 0]) cylinder(d = rod, h = rv + 1);
        // вушко штока
        color("dimgray") rotate([90, 0, 0]) difference() { cylinder(d = eye_od, h = eye_w, center = true); cylinder(d = pin, h = 100, center = true); }
        color("dimgray") translate([-eye_len, -eye_w/2, -eye_od*0.35]) cube([eye_len - pin/2, eye_w, eye_od*0.7]);
    }
}
...
module hyd_cylinder(P1, P2, bore, rod, closed, stroke, pin, wall = 5, eye_len = 45) {
    ...
    translate(p3(P1)) rotate([0, -a, 0]) {
        hyd_cyl_part("body", bore, rod, closed, stroke, pin, wall, eye_len, col_body = ok ? [0.25, 0.35, 0.6] : [0.9, 0.2, 0.2]);
...
module boom_body() {
    if (sel("tube")) color([0.85, 0.55, 0.15]) { boom_seg1(); boom_seg2(); }
}
...
module boom_bend_gussets() {
    ...
    if (sel("gusset")) color([0.55, 0.6, 0.65]) for (s = sides()) plate_xz(pts, plate_gusset, s > 0 ? boom_tube_w/2 : -boom_tube_w/2 - plate_gusset);
    // накладка на зовнішній (верхній) кут стику — лежить на полицях МІЖ щоками
    t0 = boom_tube_h/2; t1 = t0 + plate_gusset;
    if (sel("cover")) color([0.6, 0.65, 0.7]) plate_xz([K_l - u1_l*cover_half + n1_l*t0, K_l + n1_l*t0 + u1_l*(t0*tan(boom_bend/2)), K_l + u2_l*cover_half + n2_l*t0,
...
module boom_cyl_bracket_D() {
    ...
    hole = D_l;
    if (sel("D_clevis")) color([0.5, 0.55, 0.6]) for (s = sides()) let(y0 = s > 0 ? gap/2 : -gap/2 - plate_clevis) difference() {
    ...
    if (sel("D_saddle")) color([0.45, 0.5, 0.55]) plate_xz([for (p = sad) D_org + p[0]*D_u + p[1]*D_n], boom_tube_w, -boom_tube_w/2);
}
...
module boom_cyl_bracket_F() {
    ...
    if (on_apex) {
        ...
        if (sel("F_tower")) color([0.5, 0.55, 0.6]) for (s = sides()) let(y0 = s > 0 ? gap/2 : -gap/2 - plate_clevis) difference() {
        ...
        if (sel("F_clevis")) color([0.5, 0.55, 0.6]) for (s = sides()) let(y0 = s > 0 ? gap/2 : -gap/2 - plate_clevis) difference() {
            hull() { plate_xz(pts, plate_clevis, y0); ring_plate_xz(hole, r, 1, plate_clevis, y0); }
            y_hole(hole, stick_cyl_pin);
        }
        sad = [[-half - 20, top_y], [half + 20, top_y], [half + 20, sad_y], [-half - 20, sad_y]];
        if (sel("F_saddle")) color([0.45, 0.5, 0.55]) plate_xz([for (p = sad) F_org + p[0]*F_u + p[1]*F_n], boom_tube_w, -boom_tube_w/2);
    }
}

// Вісь A: втулка крізь трубу (отвір у трубі) + круглі накладки з отвором під втулку
module boom_foot_boss() {
    if (sel("A_bushing")) color("goldenrod") bushing([0, 0, 0], bush_od_main, pin_A, boom_foot_boss_len);
    if (sel("A_doubler")) color([0.5, 0.55, 0.6]) for (s = sides()) ring_plate_xz([0, 0], boom_tube_h/2 + 5, bush_od_main/2, plate_boss, s > 0 ? boom_tube_w/2 : -boom_tube_w/2 - plate_boss);
}
...
module boom_fork_B() {
    ...
    if (sel("B_fork")) color([0.5, 0.55, 0.6]) for (s = sides()) let(y0 = s > 0 ? boom_tube_w/2 : -boom_tube_w/2 - plate_clevis) difference() {
...
module stick_body() {
    if (sel("tube")) color([0.85, 0.55, 0.15]) difference() {
...
module stick_cheeks() {
    ...
    if (sel("cheek")) color([0.5, 0.55, 0.6]) for (s = sides()) let(y0 = s > 0 ? stick_tube_w/2 : -stick_tube_w/2 - plate_cheek) difference() {
        union() {
    ...
    if (sel("B_bushing")) color("goldenrod") bushing([0, 0, 0], bush_od_main, pin_B, stick_pack_w);
    // приварені розпірні втулки осі G між щоками: проміжок під вушко = clevis_gap_25
    if (sel("G_boss")) color("goldenrod") for (s = sides()) translate(p3(G_s)) rotate([90, 0, 0])
    ...
    if (sel("heel_rib")) color([0.45, 0.5, 0.55]) plate_xz([[rib0[0] - rib_w, h2], [rib0[0] + rib_w, h2], rib1 - rib_n, rib1 + rib_n], stick_tube_w, -stick_tube_w/2);
}
...
module stick_cyl_bracket_H() {
    ...
    hole = H_s;
    if (sel("H_clevis")) color([0.5, 0.55, 0.6]) for (s = sides()) let(y0 = s > 0 ? gap/2 : -gap/2 - plate_clevis) difference() {
        hull() { plate_xz(pts, plate_clevis, y0); ring_plate_xz(hole, r, 1, plate_clevis, y0); }
        y_hole(hole, bucket_cyl_pin);
    }
    if (sel("H_saddle")) color([0.45, 0.5, 0.55]) plate_xz([[bucket_cyl_sH - 80, top_y], [bucket_cyl_sH + 40, top_y], [bucket_cyl_sH + 40, sad_y], [bucket_cyl_sH - 80, sad_y]], stick_tube_w, -stick_tube_w/2);
}
...
module stick_tip_bosses() {
    ...
    if (sel("E_bushing")) color("goldenrod") bushing(p3(E_s), bushE_od, pin_E, len);
    if (sel("R_bushing")) color("goldenrod") bushing(p3(R_s), bush_od_small, pin_R, len);
    h2 = stick_tube_h/2;
    c = stick_tip_chamfer; x0 = E_s[0] - rocker_rx - 120; x1 = E_s[0] + stick_tip_ext;
    loc = [[x0, -h2], [x1 - c, -h2], [x1, -h2 + c], [x1, h2 - c], [x1 - c, h2], [x0, h2]];
    if (sel("tip_plate")) color([0.5, 0.55, 0.6]) for (s = sides()) let(y0 = s > 0 ? stick_tube_w/2 : -stick_tube_w/2 - plate_boss) difference() {
...
module washers_B() {
    ...
    if (t > 0.2) color("silver") for (s = sides()) translate([0, s*(stick_pack_w/2 + t/2), 0]) rotate([90, 0, 0])
...
module rocker_link_pts(R, J, Q, which = "both") {
    if (!is_undef(J)) {
        ...
        if (which != "link") {
            if (sel("rocker_plate")) color([0.4, 0.5, 0.45]) for (s = sides()) let(y0 = s > 0 ? w_out/2 : -w_out/2 - plate_boss) difference() {
            ...
            if (sel("J_boss")) color("goldenrod") for (s = sides()) translate(p3(J)) rotate([90, 0, 0]) translate([0, 0, s*(clevis_gap_25/2 + boss_J/2)])
                difference() { cylinder(d = pin_JQ + 16, h = boss_J, center = true); cylinder(d = pin_JQ + 0.5, h = boss_J + 2, center = true); }
        }
        if (which != "rocker")
            if (sel("link_plate")) color([0.4, 0.45, 0.55]) for (s = sides()) let(y0 = s > 0 ? w_l/2 : -w_l/2 - plate_boss) difference() {
        ...
        if (which != "rocker")
            if (sel("link_boss")) color("goldenrod") for (s = sides()) for (P = [J, Q]) bushing([P[0], s*(w_l/2 + plate_boss + link_boss_len/2), P[1]], link_boss_od, pin_JQ + 0.5, link_boss_len);
...
module bucket_sides() {     // боковини: зовнішній контур обичайки; спереду стоять НА ножі, передня кромка — від ножа до губи
    ...
    if (sel("bk_side")) color([0.78, 0.72, 0.25]) for (s = sides()) plate_xz(pts, bucket_side_t, s > 0 ? bk_w_in/2 : -bk_w_in/2 - bucket_side_t);
}
module bucket_shell() {     // обичайка між боковинами: одна вальцьована/гнута смуга
    if (sel("bk_shell")) color([0.7, 0.65, 0.22]) plate_xz(concat(bk_path(0), bk_rev(bk_path(bucket_shell_t))), bk_w_in, -bk_w_in/2);
}
module bucket_top() {       // накладка під вуха (на обичайці та кромках боковин, на всю ширину) + ребро губи всередині
    ...
    if (sel("bk_top")) color([0.6, 0.56, 0.2]) plate_xz([[bucket_top_x, y0], [bk_xsh, y0], [bk_xsh, y1], [bucket_top_x, y1]], bucket_width, -bucket_width/2);
    if (sel("bk_lip_rib")) color([0.55, 0.5, 0.18]) plate_xz([[xi, y0], [xi + bucket_lip_rib[0], y0], [xi + bucket_lip_rib[0], y0 + bucket_lip_rib[1]], [xi, y0 + bucket_lip_rib[1]]], bk_w_in, -bk_w_in/2);
}
...
module bucket_edge() {      // ніж на всю ширину + зуби (вилкою охоплюють ніж; вістря — у серединній площині ножа)
    if (sel("bk_edge")) color([0.35, 0.35, 0.38]) bucket_blade_2d_plate();
    ...
    if (sel("bk_tooth")) color([0.25, 0.25, 0.28]) for (i = [0 : ($one ? 0 : bucket_tooth_n - 1)])
...
module bucket_ears() {      // вуха стоять на накладці; зовнішні бобишки на осі E; розпірна втулка осі Q між вухами
    ...
    if (sel("bk_ear")) color([0.8, 0.62, 0.15]) for (s = sides()) let(y0 = s > 0 ? gap/2 : -gap/2 - t) difference() {
    ...
    if (sel("bk_boss_E")) color("goldenrod") for (s = sides()) bushing([0, s*(gap/2 + t + bucket_boss_E_len/2), 0], bucket_boss_E_od, pin_E + 0.5, bucket_boss_E_len);
    if (sel("bk_spacer_Q")) color("goldenrod") bushing(p3(bucket_ear), bush_od_small, pin_JQ + 0.5, gap);
}
module bucket_wear() {      // смуги зносу зовні на п'яті
    ...
    if (sel("bk_wear")) color([0.3, 0.3, 0.33]) for (i = [0 : ($one ? 0 : bucket_wear_n - 1)])
...
module ground() {
    color([0.35, 0.25, 0.15, 0.35]) translate([-1500, -1500, -ground_below_A - 20]) cube([5000, 3000, 20]);
}
