...
| `tools/check_personal.py` | **Before every commit**: the staged changes are searched for personal data — the added text and the metadata inside binaries (PDF, PNG text chunks, zip-based files such as `.3mf`). The machine-specific patterns (home folder, user name, git e-mail) are read at run time; none of them is stored in the script. |
| `tools/check_docs.py` | After editing the docs: every relative link and image leads to an existing file, every `#anchor` to a real heading, and the English/Ukrainian twins (README, TECHNICAL, QUICKSTART, SAFETY) have the same skeleton. |
...
| `.claude/skills/` | The project skills: `scad-mechanism` (model rules and checklist), `new-assembly` (the recipe for a new sub-assembly), `build-version`, `part-drawings`, `browser-verify` (checking the pages in headless Chrome), `media-kit` (material for publications), `print-kit` (the 1:5 printed kit), `print-sheets` (the 1:1 sorting sheets), `design-review`, `research-sweep`, `agent-analytics`. Everything stays inside the repository; nothing is installed into `~/.claude`. |
