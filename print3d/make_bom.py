...
def main():
    ...
    # НА ПОТІМ: номер версії тут зашитий, і це єдине в BOM, що не виміряне.
    # Обидва набори підключають ЖИВУ модель (`include <../scad/excavator_boom.scad>`),
    # а не знімок із versions/ — тобто з версією вони пов'язані лише цим підписом
    # (ще двічі в print3d-parts/README.md: шлях до DXF і проза про походження набору).
    # Щойно з'явиться V002, підпис почне брехати. Тоді винести його в аргумент
    # make.sh (тека найбільшої версії) і перегенерувати обидва BOM.
