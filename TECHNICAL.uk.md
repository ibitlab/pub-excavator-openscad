...
| `print3d-parts/` | **Друкований набір 1:5** — кожен компонент окремо, як його ріжуть з металу (54 файли, 76 штук), склеюється замість зварювання: `make.sh` → STL по завданнях слайсера, `BOM.md`, `ASSEMBLY.md`. Окремо `sheets/SHEETS.pdf` — **аркуші розкладки 1:1** для сортування купи надрукованого: контур кожної деталі на аркуші свого вузла, рендери з виносками, картки кроків склеювання. Див. розділ 2. |
...
| `tools/page_style.py` | **Один вигляд усіх PDF**: шапка (жирний заголовок, «набір · версія · дата», лінія 0.5 мм) і підвал (документ · автор · дата · с. N / M) у `SHEETS.pdf`, `ASSEMBLY.pdf` і ескізах деталей. |
...
```
print3d-parts/make.sh                                              # STL усіх компонентів, перевірка, BOM, ASSEMBLY, аркуші розкладки
