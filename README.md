...
- Initial parameter values can be given in the URL: `?boom_L1=1000&view=iso`. The model is embedded into the page at build time, and **there is no way to open somebody else's `.scad` in it** — that was removed on purpose: such a file's text would become the group names, descriptions and warnings in the panel, i.e. foreign data inside a page that lives on a shared GitHub Pages domain.
