...
- **The machine can be dragged by the bucket.** Grab the bucket body and the whole arm follows your finger: the page solves the inverse kinematics and works out the boom and stick angles. Grab the teeth and the bucket curls about its own axis while the arm stays put. Works with a mouse and with a single finger. The solution is exact and needs no iteration — joint B lies on the intersection of two circles — and `npm test` runs the forward problem through the inverse one and back (tooth error 10⁻¹² mm). An unreachable target does not freeze the arm: it follows to the edge of the working envelope, and the cylinder-stroke limit still applies. The "Lock the camera" button disables orbiting altogether, so you can then drag from anywhere without aiming at the bucket. A pill above the model points this out **exactly once**: it fades away after the first drag (or when dismissed) and never returns, so it costs no permanent line in the panel.
