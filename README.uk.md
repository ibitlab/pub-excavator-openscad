...
Кожен компонент можна надрукувати в масштабі 1:5 і склеїти там, де в металі був би шов: 54 файли, 76 штук. Для склеювання й зроблено аркуші розкладки 1:1 вище; як набір поділено й друкується — [print3d-parts/README.md](print3d-parts/README.md).

## Як це зроблено

**Ihor Ivaniuk (@IBITLAB)** — ідейник і власник продукту. Від автора — ідея машини (причіпний міні-екскаватор під уже закуплені гідроциліндри), кожна задача, рішення, що лишається, а що відкидається, перевірка результатів — на рендерах, на 3D-сторінці, на надрукованому наборі в руках — і сама організація роботи ШІ: які перевірки мусять бути, яким має бути кожен документ, що публікується. Це багато годин роботи в багатьох сесіях. Як усе почалося — словами автора й з першими промптами: [STORY.uk.md](STORY.uk.md).

**Виконавець — ШІ**, Claude Code від Anthropic: модель OpenSCAD і її незалежні двійники на Python (кінематика, міцність, ківш), креслення, специфікація й DXF, обидві 3D-сторінки, друкований набір 1:5 з аркушами розкладки, тексти двома мовами. «Незалежні перевірки» — теж ШІ-агенти з різними лінзами; їхні знахідки йдуть до автора, а що виправляти — рішення автора.

**Інструменти:** OpenSCAD (рушій Manifold) · Python (numpy, shapely, matplotlib, Pillow) · three.js · OpenSCAD, скомпільований у WebAssembly, + Vite · headless Chrome (перевірка сторінок, PDF) · poppler · ffmpeg.

Нічого з цього не перевіряв кваліфікований інженер — див. застереження нижче.

## Застереження

> ## ⚠️ Зроблено за допомогою ШІ. Використання — на власний ризик
>
> **Ідея, задачі й рішення — автора (Ihor Ivaniuk); уся ця документація, геометрія моделі, розрахунки, креслення й специфікації створені та прораховані штучним інтелектом (Claude Code) за цими задачами.** Їх не перевіряв і не затверджував кваліфікований інженер. «Незалежна перевірка» в розділі 4а [TECHNICAL.uk.md](TECHNICAL.uk.md) — теж робота ШІ-агентів, а не людей-фахівців.
>
