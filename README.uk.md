...
| `print3d-parts/` | **Друкований набір 1:5** — кожен компонент окремо, як його ріжуть з металу (53 файли, 76 штук), склеюється замість зварювання: `make.sh` → STL по завданнях слайсера, `BOM.md`, `ASSEMBLY.md`. Окремо `sheets/SHEETS.pdf` — **аркуші розкладки 1:1** для сортування купи надрукованого: контур кожної деталі на аркуші свого вузла, рендери з виносками, картки кроків склеювання. Див. розділ 2. |
