...
| Друкований набір: інструкція й аркуші розкладки на папір (`print3d-parts/make.sh`) | **Chrome** (HTML → PDF) і **poppler** (`brew install poppler`: `pdftoppm`, `pdfinfo`, `pdftotext` — прев'ю та перевірка сторінок із самого PDF; без нього прев'ю робляться знімком HTML) |
