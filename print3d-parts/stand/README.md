...
- **від висунення — обхватом і натягом рукава на штоку:** рукав 5 мм обіймає шток на
  245° (рукав гільзи — на 225°) і сидить із натягом `CLIP_FIT = -0.15` (FDM ще й сам
  звужує дуги на 0.1–0.2 мм). Це тертя, не замок: на витягування прищепка тримає
...
але скруглення з'їдає ~9° обхвату з кожного боку — тому рукав штока ширший: 245° реально
обіймають ~227° (225° давали б лише ~207°, на 13° за півколо, і шток вислизав). Губки
при защіпанні розходяться на ~9 % на штоку й ~5 % на гільзі — PLA витримує, PETG
пружніший. У `stand.scad`: спадає зі штока — `CLIP_ROD_ARC` ще на 5–10° або `CLIP_FIT`
ще на −0.05; туго на гільзі — `CLIP_FIT` ближче до 0; штуцер не входить — збільшити
`CLIP_PORT_FIT`.
...
циліндрі з набору зі штоком на 40 % ходу: при номінальному зазорі (`CLIP_FIT = 0`;
від'ємний — навмисний натяг) з гільзою і штоком не перетинається, а без
