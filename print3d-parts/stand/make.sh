...
# при номінальному зазорі: від'ємні CLIP_FIT і CLIP_ROD_FIT — навмисний натяг, він перетин і дає
render clip_fit /tmp/stand_clip_fit.stl -D CLIP_FIT=0 -D 'CLIP_ROD_FIT=[0,0]'
v=$(vol /tmp/stand_clip_fit.stl)
[ "$v" = "0.000" ] || { echo "  ПОМИЛКА: прищепка перетинає гільзу або шток — $v см³"; exit 1; }
echo "  з гільзою і штоком не перетинається: так (перетин 0.000 см³ при нульових зазорах)"
render clip_seat /tmp/stand_clip_seat.stl -D CLIP_FIT=0 -D 'CLIP_ROD_FIT=[0,0]'
