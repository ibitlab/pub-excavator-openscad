...
CLIP_ROD_L = 5;
CLIP_BODY_L = [10, 8];
// Кут обхвату рукава на гільзі і переходу: губки при защіпанні розходяться на ~5 % — PLA витримує
CLIP_ARC = 225;
// Кут обхвату рукава на штоку — більший: на штоку замка немає, тримає лише обхват і натяг.
// Скруглення губок на штоку Ø8 з'їдає ~9° з кожного боку: 245 обіймає реально ~227°
// (225 — лише ~207°, тобто на 13° за півколо). Щілина між скругленнями на ~9 % вужча
// за шток; гнеться лише тонкий рукав 5 мм, товстий перехід розходиться як і раніше
CLIP_ROD_ARC = 245;
// Стінка рукавів (4 лінії сопла)
CLIP_WALL = 1.6;
// Діаметральний зазор рукавів: 0 — у номінал (FDM сам звужує дуги на 0.1–0.2, і сидить
// з натягом); мінус — натяг, спадає — ще −0.05, туго на гільзі — ближче до 0
CLIP_FIT = -0.15;
