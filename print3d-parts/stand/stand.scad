...
CLIP_ROD_L = 5;
CLIP_BODY_L = [10, 8];
// Кут обхвату рукава на гільзі і переходу: губки при защіпанні розходяться на ~8 % — PLA витримує
CLIP_ARC = 225;
// Кут обхвату рукава на штоку — більший: на штоку замка немає, тримає лише обхват і натяг.
// Скруглення губок з'їдає ~9° з кожного боку на Ø8 і ~13° на Ø5: 260 обіймає реально
// ~242° і ~233° (225 на Ø8 — лише ~207°). Губки розходяться на ~23 % і ~17 %; гнеться
// лише тонкий рукав 5 мм, товстий перехід — як на гільзі
CLIP_ROD_ARC = 260;
// Стінка рукавів (4 лінії сопла)
CLIP_WALL = 1.6;
// Діаметральний зазор рукава на гільзі: 0 — у номінал (FDM сам звужує дуги на 0.1–0.2,
// і сидить з натягом); мінус — натяг, туго — ближче до 0. Там і так тримає штуцер
CLIP_FIT = -0.2;
// Зазор рукава на штоку (стріла / решта): при однаковій стінці рукав на Ø8 тисне втричі
// слабше, ніж на Ø5 (жорсткість кільця ~1/R³), тож стрілі — подвійний натяг; спадає —
// ще −0.05…−0.1
CLIP_ROD_FIT = [-0.4, -0.2];
...
function stand_plate_edge(z) =
  let (
    c1 = [0, stand_lobe_z()],
    r1 = 55 / SC,
    c2 = C_w / SC + [0, STAND_A_H],
    r2 = 45 / SC,
    d = c2 - c1,
    a = atan2(d[1], d[0]) - acos((r1 - r2) / norm(d)),
    n = [cos(a), sin(a)]
...
module stand_base() {
  ...
  difference() {
    ...
    translate([0, 0, -eps]) cylinder(
        d1=STAND_SCREW_HEAD_D + 2 * eps, d2=STAND_SCREW_D,
...
module stand_post() {
  ...
  render(convexity=8) difference() {
      ...
      for (s = [-1, 1])
        translate([0, s * (STAND_W / 2 + STAND_POCKET), 0]) rotate([90, 0, 0])
...
module clip_ring_2d(ri, ro, arc = CLIP_ARC) {
  w = ro - ri;
  g = (360 - arc) / 2;
...
// (з урахуванням зазорів: різний натяг на штоку й гільзі інакше робить конус положистим)
function clip_step(i) = let (s = clip_sizes()[i]) CLIP_ROD_L + (s[1] + CLIP_FIT - s[0] - CLIP_ROD_FIT[i]) / 2;
...
module clip_body(i = CLIP_I) {
  ...
  z2 = clip_step(i);
  union() {
    linear_extrude(z1 + eps) clip_ring_2d(ri_r, ro_r, CLIP_ROD_ARC);
...
module clip_demo_cyl(f = 0.4) {
  ...
  scale(1 / SC) {
    ...
    translate([L, 0, 0]) hyd_cyl_part(
        "rod", boom_cyl_bore, boom_cyl_rod, boom_cyl_closed, boom_cyl_stroke,
...
module clip_demo_placed(hole = true) {
  ...
  translate([x_end + clip_step(0), 0, 0]) rotate([0, -90, 0]) rotate([0, 0, 180]) if (hole) stand_clip(0); else clip_body(0);
}
...
else if (what == "clip_fit")
  intersection() { clip_demo_placed(); clip_demo_cyl(); }
// …а без отвору під штуцер перетинається саме зі штуцером (має бути > 0)
else if (what == "clip_seat")
  intersection() { clip_demo_placed(hole=false); clip_demo_cyl(); }
// перетин плит зі стійкою — має бути порожнім (OpenSCAD тоді не пише файл)
else if (what == "fit")
  intersection() { translate([0, 0, STAND_BASE_T]) stand_post(); stand_plates(); }
// перетин плит із тілом без виїмок — має бути > 0: виїмки саме там, де плити
else if (what == "seat")
  intersection() { translate([0, 0, STAND_BASE_T]) stand_post_body(); stand_plates(); }
else {
